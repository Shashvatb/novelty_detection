...
def run_mem_net(train_data, test_data, seed=1, word_vector_size=50,
    ...
    if network == 'dmn_basic':
        ...
        if batch_size != 1:
            ...
            batch_size = 1
        dmn = model.DMN_basic(
            **args_dict)  # Initialize the dmn basic with all the arguments available. This also initializes theano functions and parameters.
    ...
    for epoch in range(epochs):
        ...
        avg_loss, skipped, perform_results, y_pred = do_epoch(dmn, 'train', epoch, batch_size,
                                                              log_every)  # Run do_epoch for train

        epoch_loss, skipped, perform_results, y_pred = do_epoch(dmn, 'test', epoch, batch_size, log_every,
                                                                skipped)  # Run do_epoch for test
...
def do_epoch(dmn, mode, epoch, batch_size, log_every, skipped=0):
    ...
    perform_results = utils.get_perform_metrics(y_true, y_pred)
