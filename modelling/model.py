...
class DMN_basic:
    ...
    def __init__(self, train_raw, test_raw, word2vec, word_vector_size,
        ...
        for it in range(1, self.memory_hops + 1):
            current_episode = self.new_episode(memory[it - 1])
            memory.append(self.GRU_update(memory[it - 1], current_episode,
        ...
        self.params = [self.W_inp_res_in, self.W_inp_res_hid, self.b_inp_res,
                       ...
                       self.W_mem_upd_in, self.W_mem_upd_hid, self.b_mem_upd,
                       self.W_mem_hid_in, self.W_mem_hid_hid, self.b_mem_hid,
                       self.W_b, self.W_1, self.W_2, self.b_1, self.b_2, self.W_a, self.net_w]
    ...
    def load_state(self, file_name):
        ...
        with open(file_name, 'rb') as load_file:
            dic = pickle.load(load_file)
            loaded_params = dic['params']
